...
**1. What is your robot's goal?** Work like the parking assistant, drive forward slowly and beep when it gets closer, then stopping if it gets too close to the object.

**2. What should the robot do when the sensor reads a SHORT distance?** (object is close)
It should be moving very slowly and beeping loudly, and possibly be stopped.

**3. What should the robot do when the sensor reads a LONG distance?** (nothing nearby)
Slowly beep and continue backing up.

**4. What distance threshold(s) will you use?** (e.g., 200 mm, 500 mm — you may need more than one)
50mm is stopped
50-250 are progressive.
...
Example:
  ...
  - Loop forever:
      - Read the sensor
      - If distance < 150 mm:
          - Stop, beep, turn right 90 degrees
      - Else:
...
Pseudocode:
    - Loop Forever:
    - Read distance
    - If object is detected within 250mm:
        - Beep 
        - Make beeping frequency = total meters * 0.01 <--- (or whatever number works, this will be adjusted)
    - Else:
        - Keep driving
...
**Requirements:**
- [X] Uses `UltrasonicSensor` to read distance
- [X] Uses `DriveBase` to move
- [X] Has a `while` loop with sensor readings
- [X] Has at least one `if/else` decision based on the sensor
- [X] Has comments explaining the logic
- [X] Displays something useful on the EV3 screen (distance, status, etc.)
